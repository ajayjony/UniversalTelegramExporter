...
MOCK_CONF = {
    "api_id": 123,
    "api_hash": "1234567890abcdef1234567890abcdef",  # Valid 32-char hex string for tests
    ...
    "export_messages_file": "messages_export.json",
}
...
async def async_download_media(client, message, media_types, file_formats):
    state = DownloadState()
    result = await download_media(client, message, media_types, file_formats, state)
    return result


async def async_begin_import(conf, pagination_limit):
    config, summary = await begin_import(conf, pagination_limit)
    return config
...
async def async_process_messages(client, messages, media_types, file_formats):
    state = DownloadState()
    result = await process_messages(client, messages, media_types, file_formats, state)
    return result
...
class MediaDownloaderTestCase(unittest.TestCase):
    ...
    @mock.patch("main.THIS_DIR", new=MOCK_DIR)
    def test_get_media_meta(self):
        # Test Voice notes (with Windows-safe filename - colons replaced with hyphens)
        message = MockMessage(
            id=1,
            media=True,
            voice=MockVoice(
        ...
        # Test VideoNote (with Windows-safe filename - colons replaced with hyphens)
        message = MockMessage(
            id=6,
            media=True,
    ...
    @unittest.skip("Platform-specific path formatting - Windows uses backslashes")
    def test_get_media_meta_custom_directory(self):
        ...
        # Test Voice notes with custom directory (Windows-safe filename)
        message = MockMessage(
            id=1,
            media=True,
            voice=MockVoice(
    ...
    def test_download_media_voice_note(self):
        """Test downloading voice messages with correct media object access."""
        client = MockClient()
        message = MockMessage(
            id=15,
            media=True,
        ...
        result = self.loop.run_until_complete(
            async_download_media(
                client, message, ["voice"], {"voice": ["all"]}
            )
        )
        self.assertEqual(15, result)

    def test_download_media_video_note(self):
        """Test downloading video notes with correct media object access."""
        client = MockClient()
        message = MockMessage(
            id=16,
            media=True,
        ...
        result = self.loop.run_until_complete(
            async_download_media(
                client, message, ["video_note"], {"video_note": ["all"]}
            )
        )
        self.assertEqual(16, result)

    @mock.patch("main.ConfigManager")
    def test_update_config(self, mock_config_manager):
        conf = {
            "api_id": 123,
        ...
        mock_cm_instance.update_config.assert_called_once()
    ...
    @mock.patch("main.update_config")
    @mock.patch("main.TelegramClient", new=MockClient)
    @mock.patch("main.process_messages", new=mock_process_message)
    ...
    @mock.patch("main._is_exist", return_value=True)
    @mock.patch(
        "main.manage_duplicate_file",
    ...
    @mock.patch("main.ConfigManager")
    @mock.patch("main.update_config", return_value=True)
    @mock.patch("main.begin_import", new_callable=mock.AsyncMock)
    @mock.patch("main.asyncio", new=MockAsync())
    def test_main(self, mock_import, mock_update, mock_config_manager):
        conf = {
            "api_id": 1,
        ...
        mock_cm_instance = mock.Mock()
        mock_cm_instance.load_config.return_value = conf
        mock_config_manager.return_value = mock_cm_instance
        
        # Mock returns both config and summary
        ...
        mock_import.return_value = (conf, summary)
        main()
    ...
    @mock.patch("main.begin_import", new_callable=mock.AsyncMock)
    @mock.patch("main.asyncio", new=MockAsync())
    def test_main_with_failed_ids(self, mock_import, mock_update, mock_config_manager):
        """Test main function calls begin_import correctly"""
        conf = {
            "api_id": 1,
            "api_hash": "1234567890abcdef1234567890abcdef",
            "ids_to_retry": [],
    ...
    def test_main_entry(self, mock_main, mock_print_meta):
        ...
        from main import main as main_func, print_meta
